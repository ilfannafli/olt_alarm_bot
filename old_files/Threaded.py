...
def func():
    with ThreadPoolExecutor(max_workers=2) as executor:
        for olt in data:
            executor.submit(thread_function, olt)

def thread_function(olt):
       ...
       # print(cekpwd.decode())
        ...
        if olt[1] == "AN5516-04": 
            ...
            for command in cmd:
                ...
                match = re.search(r"total_load_current (\d+\.\d+)", str(output2))
                current_load = float(match.group(1)) 
                match2 = re.search(r"battery_capacity (\d+\.\d+)", str(output2))
        ...
        cursor = connection.cursor()
        sql_insert_table_warning = 'INSERT INTO `test_dropvoltage`(`ip`,`voltage`,`current_load`,`Percentage`,`capacity`) VALUES ("'+olt[0]+'","'+voltase+'","'+current_load+'","'+bat_percentage+'","'+sisabaterai+'");'
